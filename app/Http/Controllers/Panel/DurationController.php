...
class DurationController extends Controller
{
    ...
    {
        $request = Duration::findOrFail($id);

        if ($this->deleteAction($request)) {
            return redirect()->route('panel.duration.index')->with('success', 'Duration deleted successfully');
        } else {
            return redirect()->route('panel.duration.index')->with('success', 'Duration deleted successfully');
        }
    }

    public function deleteAction(Duration $request)
    {
